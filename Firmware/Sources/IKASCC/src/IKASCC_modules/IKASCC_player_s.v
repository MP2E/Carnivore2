...
reg     [7:0]   db_z;
wire            scc_reg_wr = RAMCTRL_ASYNC
                            ? (~i_CS_n & ~i_WR_n & i_SCCREG_EN)
                            : (i_WRRQ & i_SCCREG_EN);
wire            test_wr = scc_reg_wr & (i_ABLO[7:5] == 3'b111);
wire            mode_wr = scc_reg_wr & (i_ABLO[7:5] == 3'b110);
reg     [7:0]   test;
always @(posedge emuclk) begin
    if(!rst_n) test <= 8'h00;
    else begin if(!mclkpcen_n) begin
        db_z <= i_DB;
        if(mode_wr | test_wr) test <= RAMCTRL_ASYNC ? i_DB : db_z;
    end end
end
...
wire            scc_classic = ~i_SCC_PLUS_MODE;

wire signed     [7:0]   ch1_sound, ch2_sound, ch3_sound;
wire signed     [7:0]   ch4_sound, ch5_sound;
wire signed     [7:0]   ch4_sound_classic, ch5_sound_classic;
wire signed     [7:0]   ch4_sound_plus, ch5_sound_plus;
assign  ch4_sound = i_SCC_PLUS_MODE ? ch4_sound_plus : ch4_sound_classic;
assign  ch5_sound = i_SCC_PLUS_MODE ? ch5_sound_plus : ch5_sound_classic;
always @(posedge emuclk) begin
...
IKASCC_player_control_s #(
    ...
    .i_SCCPLUS_ADDR             (i_SCC_PLUS_MODE            ),
...
///////////////////////////////////////////////////////////
//////  Channel 4 and 5 - classic SCC (shared wave RAM)
////
...
always @(posedge emuclk) begin
    ...
    else begin if(!mclkpcen_n && scc_classic) begin
...
// Classic SCC compat: read channel 5 wave via 98A0-98BF (ABLO[7:5]==101), no writes.
reg     [7:0]   i_ablo_z;
always @(posedge emuclk) if(!mclkpcen_n) i_ablo_z <= i_ABLO;

wire            ch5_compat_rdrq = scc_classic && (
                                RAMCTRL_ASYNC
                                    ? (~i_CS_n & ~i_RD_n & i_SCCREG_EN & (i_ABLO[7:5] == 3'b101))
                                    : (i_RDRQ & i_SCCREG_EN & (i_ablo_z[7:5] == 3'b101))
                                );
wire            ch45_host_rdrq = ch45_ram_rdrq | ch5_compat_rdrq;
wire    [4:0]   ch45_host_rdaddr = ch5_compat_rdrq
                                ? (RAMCTRL_ASYNC ? i_ABLO[4:0] : i_ablo_z[4:0])
                                : ch45_ram_addr_cpu;

always @(*) begin
    if(ch45_ram_wrrq | ch45_host_rdrq)
        ch45_ram_addr = ch45_host_rdaddr;
    else case(ch45_ram_addrsel)
...
always @(posedge emuclk) if(!mclkpcen_n) begin
    if(scc_classic && ch4_wavelatch_tick_pcen) ch4_wavelatch <= ch45_ram_q;
    if(scc_classic && ch5_wavelatch_tick_pcen) ch5_wavelatch <= ch45_ram_q;
end
...
IKASCC_player_memory_s #(.RAM_TYPE(RAM_TYPE), .INITFILE()) u_mem_ch45 (
    ...
    .i_RAM_WRRQ                 (ch45_ram_wrrq & ~test[6] & ~test[7] & scc_classic),
...
IKASCC_player_control_s #(
    ...
    .o_SOUND                    (ch5_sound_classic          )
);



///////////////////////////////////////////////////////////
//////  Channel 4 and 5 - SCC+ (private wave RAM each)
////

wire            ch4p_ram_rdrq, ch4p_ram_wrrq;
wire            ch5p_ram_rdrq, ch5p_ram_wrrq;
wire    [4:0]   ch4p_ram_addr_cntr, ch4p_ram_addr_cpu;
wire    [4:0]   ch5p_ram_addr_cntr, ch5p_ram_addr_cpu;
wire    [4:0]   ch4p_ram_addr = ch4p_ram_rdrq ? ch4p_ram_addr_cpu : ch4p_ram_addr_cntr;
wire    [4:0]   ch5p_ram_addr = ch5p_ram_rdrq ? ch5p_ram_addr_cpu : ch5p_ram_addr_cntr;
wire    [7:0]   ch4p_ram_d, ch4p_ram_q;
wire    [7:0]   ch5p_ram_d, ch5p_ram_q;

IKASCC_player_memory_s #(.RAM_TYPE(RAM_TYPE), .INITFILE()) u_mem_ch4p (
    .i_EMUCLK                   (emuclk                     ),
    .i_MCLK_PCEN_n              (mclkpcen_n                 ),

    .i_RAM_WRRQ                 (ch4p_ram_wrrq & i_SCC_PLUS_MODE),
    .i_RAM_ADDR                 (ch4p_ram_addr              ),
    .i_RAM_D                    (ch4p_ram_d                 ),
    .o_RAM_Q                    (ch4p_ram_q                 )
);

IKASCC_player_memory_s #(.RAM_TYPE(RAM_TYPE), .INITFILE()) u_mem_ch5p (
    .i_EMUCLK                   (emuclk                     ),
    .i_MCLK_PCEN_n              (mclkpcen_n                 ),

    .i_RAM_WRRQ                 (ch5p_ram_wrrq & i_SCC_PLUS_MODE),
    .i_RAM_ADDR                 (ch5p_ram_addr              ),
    .i_RAM_D                    (ch5p_ram_d                 ),
    .o_RAM_Q                    (ch5p_ram_q                 )
);
...
IKASCC_player_control_s #(
    ...
    .o_RAM_RDRQ                 (ch4p_ram_rdrq              ),
    .o_RAM_WRRQ                 (ch4p_ram_wrrq              ),
    .o_RAM_ADDR_CNTR            (ch4p_ram_addr_cntr         ),
    .o_RAM_ADDR_CPU             (ch4p_ram_addr_cpu          ),
    .o_RAM_D                    (ch4p_ram_d                 ),
    .i_RAM_Q                    (ch4p_ram_q                 ),

    .o_FRACCNTR_LD_n            (                           ),

    .o_SOUND                    (ch4_sound_plus             )
);
...
IKASCC_player_control_s #(
    ...
    .o_RAM_RDRQ                 (ch5p_ram_rdrq              ),
    .o_RAM_WRRQ                 (ch5p_ram_wrrq              ),
    .o_RAM_ADDR_CNTR            (ch5p_ram_addr_cntr         ),
    .o_RAM_ADDR_CPU             (ch5p_ram_addr_cpu          ),
    .o_RAM_D                    (ch5p_ram_d                 ),
    .i_RAM_Q                    (ch5p_ram_q                 ),

    .o_FRACCNTR_LD_n            (                           ),

    .o_SOUND                    (ch5_sound_plus             )
);
...
always @(*) begin
    ...
    else if(i_SCC_PLUS_MODE && ch4p_ram_rdrq) o_DB = ch4p_ram_q;
    else if(i_SCC_PLUS_MODE && ch5p_ram_rdrq) o_DB = ch5p_ram_q;
    else if(scc_classic && ch45_host_rdrq) o_DB = ch45_ram_q;
end
...
wire    [7:0]   eff_freq_base = ADDR_FREQ_BASE + (i_SCCPLUS_ADDR ? 8'h20 : 8'h00);
wire    [7:0]   eff_vol_addr  = ADDR_VOL + (i_SCCPLUS_ADDR ? 8'h20 : 8'h00);
wire    [7:0]   eff_mute_addr = ADDR_MUTE + (i_SCCPLUS_ADDR ? 8'h20 : 8'h00);
...
wire    freq_lo_wr = scc_reg_wr & (i_ABLO[7:5] == eff_freq_base[7:5]) & (i_ABLO[3:0] ==  eff_freq_base[3:0]);
wire    freq_hi_wr = scc_reg_wr & (i_ABLO[7:5] == eff_freq_base[7:5]) & (i_ABLO[3:0] == (eff_freq_base[3:0] + 4'h1));
reg     [11:0]  freq;
always @(posedge emuclk) if(!mclkpcen_n) begin
    if(freq_lo_wr) freq[7:0] <= RAMCTRL_ASYNC ? i_DB : i_DB_Z;
    if(freq_hi_wr) freq[11:8] <= RAMCTRL_ASYNC ? i_DB[3:0] : i_DB_Z[3:0];
end
...
wire            freq_changed;
assign  freq_changed = (~i_WR_n & i_SCCREG_EN & (i_ABLO[7:5] == eff_freq_base[7:5]) & (i_ABLO[3:0] ==  eff_freq_base[3:0])) |
                       (~i_WR_n & i_SCCREG_EN & (i_ABLO[7:5] == eff_freq_base[7:5]) & (i_ABLO[3:0] == (eff_freq_base[3:0] + 4'h1)));
...
wire            mute_wr = scc_reg_wr & (i_ABLO[7:5] == eff_mute_addr[7:5]) & (i_ABLO[3:0] ==  eff_mute_addr[3:0]);
reg             mute;
always @(posedge emuclk) begin
    if(!i_RST_n) mute <= 1'b0;
    else begin if(!mclkpcen_n) begin
        if(mute_wr) mute <= RAMCTRL_ASYNC ? i_DB[BIT_MUTE] : i_DB_Z[BIT_MUTE];
    end end
end
...
//volume register
wire    vol_wr = scc_reg_wr & (i_ABLO[7:5] == eff_vol_addr[7:5]) & (i_ABLO[3:0] ==  eff_vol_addr[3:0]);
reg     [3:0]  vol;
always @(posedge emuclk) if(!mclkpcen_n) begin
    if(vol_wr) vol <= RAMCTRL_ASYNC ? i_DB[3:0] : i_DB_Z[3:0];
end
...
always @(posedge emuclk) if(!mclkpcen_n) begin
    ...
    if(mute_wr && ~(RAMCTRL_ASYNC ? i_DB[BIT_MUTE] : i_DB_Z[BIT_MUTE])) final_sound <= 8'h00;
...
//declare wavetable RAM (INITFILE/$readmemh omitted: Quartus requires a literal path)
